...
struct KatakanaDetailView: View {
    ...
    var body: some View {
        ZStack {
            Color(red: 0.1, green: 0.1, blue: 0.2)
                .ignoresSafeArea()
            VStack {
                Text(katakana[JapaneseID].letter)
                    .foregroundColor(.white)
                    .font(.system(size: 150))
                
                Button {
                    soundplayer.playSounds(file: katakana[JapaneseID].voice1)
                } label: {
                ...
                HStack {
                    ...
                    Button {
                        soundplayer.playSounds(file: katakana[JapaneseID].voice2)
                    } label: {
                        Image(systemName: "speaker.wave.3.fill")
                            .font(.system(size: 25))
                    }
